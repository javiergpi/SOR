...
# Si tu número es menor que 10, no anteponer 0
NN=66
...
#Nombre del PDC 
Nombre_Servidor="SRV-EXAMEN-$NN"

#Nombre DNS del dominio 
DNS_Dominio="examen$NN.local"

#Nombre NETBIOS del dominio
# Ejemplo DOMINIO00
NETBIOS_Dominio="EXAMEN$NN"

###########################################################
# VARIABLES CLIENTE

#Nombre del CLIENTE
Nombre_Cliente="PC-EXAMEN-$NN"
...
AWS_ID_VPC=$(aws ec2 create-vpc \
  ...
  --tag-specifications 'ResourceType=vpc,Tags=[{Key=Name,Value=SOR-EXAMEN-vpc}]' \
...
AWS_ID_SubredPublica=$(aws ec2 create-subnet \
  ...
  --tag-specifications 'ResourceType=subnet,Tags=[{Key=Name,Value=SOR--EXAMEN-subred-publica}]' \
...
AWS_ID_InternetGateway=$(aws ec2 create-internet-gateway \
  --tag-specifications 'ResourceType=internet-gateway,Tags=[{Key=Name,Value=SOR-EXAMEN-igw}]' \
...
--tags "Key=Name,Value=SOR-EXAMEN-rtb-public"
...
AWS_DHCP_OPTIONS_ID=$(aws ec2 create-dhcp-options \
    ...
    --tag-specifications 'ResourceType=dhcp-options,Tags=[{Key=Name,Value=SOR-EXAMEN-DHCP-opciones}]' \
...
AWS_CUSTOM_SECURITY_GROUP_ID=$(aws ec2 describe-security-groups \
  --filters "Name=vpc-id,Values=$AWS_ID_VPC" \
  --query 'SecurityGroups[?GroupName == `SOR-EXAMEN-Windows-SG`].GroupId' \
  --output text)
...
--tags "Key=Name,Value=SOR-EXAMEN-Windows-SG" 
...
--tags "Key=Name,Value=SOR-SERVIDOR-EXAMEN-ip" 
...
--tags "Key=Name,Value=SOR-CLIENTE-EXAMEN-ip" 
